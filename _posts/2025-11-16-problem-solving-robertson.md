...
A problem exists when there's a gap between where you are and where you want to be — and how to close that gap isn't immediately obvious. The "where you want to be" is always an imagined state: something you'd like to be true that isn't yet. Problem solving is "what you do, when you don't know what to do." The moment you know what to do, the problem has dissolved into a task.

That second condition — *not knowing how* — is what makes the definition non-trivial. A goal without a path is a problem. A goal with a known path is just execution. In practice, the gap between them is often murkier than it looks: sometimes the goal itself is ill-defined, and clarifying it *is* the problem. "It is foolish to answer a question that you do not understand. It is sad to work for an end that you do not desire." Both failure modes are common and often invisible — you can be deep in a solution before noticing you've misread the goal or accepted a goal you never actually wanted.
...
Because recall is reconstruction, encoding strategies like elaboration and dual coding help by laying down richer cues to rebuild from, not by storing a cleaner recording. And reconstruction has a second consequence that reaches past memory into problem solving itself: what you can later retrieve is governed by surface resemblance, not structure. That is where transfer succeeds or fails.
